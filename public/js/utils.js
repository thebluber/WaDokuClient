...
var utils = {
  ...
  renderResults(results) {
    ...
    for(var i=0; i<results.length; i++) {
      $('<a class="result brand" onclick="utils.dispatchCopyEvent(this.innerHTML)">' + results[i] + '</a>').appendTo('#results');
    }
  },
  renderResults4Phone(results) {
    $('#results-phone').html('');
    for(var i=0; i<results.length; i++) {
      $('<a class="result" onclick="utils.dispatchCopyEvent(this.innerHTML)">' + results[i] + '</a>').appendTo('#results-phone');
    }
  },
  renderTimer(time) {
  ...
  dispatchCopyEvent(value){
    var evt = new CustomEvent('copy', { 'detail': value });
    window.dispatchEvent(evt);
  },
  copy(value){
    $('#search-top')[0].value += value;
  }
};
